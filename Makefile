...
# Kernel-style seeding of an output directory: the target stem names the
# platform, and the VM configuration is then set via menuconfig
...
# Dependency files are only read once auto.conf is current: remaking them
# runs the generators, which need the configuration it resolves
ifneq ($(build_targets),)
ifeq ($(kconfig_auto_stale),)
-include $(deps)
endif
