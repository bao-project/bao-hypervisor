...
inline deviceid_t smmu_sme_get_id(size_t sme)
{
    return SMMU_SMR_ID(smmu.hw.glbl_rs0->SMR[sme]);
}

inline deviceid_t smmu_sme_get_mask(size_t sme)
{
                                ...
                                bool group)
{
    ...
    {
        deviceid_t sme_mask = smmu_sme_get_mask(sme);
        deviceid_t mask_r = mask & sme_mask;
        deviceid_t diff_id = (smmu_sme_get_id(sme) ^ id) & ~(mask | sme_mask);
