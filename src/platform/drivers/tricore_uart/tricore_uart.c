...
{
    volatile struct PROT_ACCESSEN* uart_accessen = &(uart->ACCEN);
    struct PROT_ACCESSEN* gpio_accessen = (struct PROT_ACCESSEN*)(PORT14 + 0x90);
    apu_enable_access_cpu(uart_accessen, 0UL);
    apu_enable_access_cpu(uart_accessen, 1UL);
    apu_enable_access_cpu(gpio_accessen, 0UL);
    apu_enable_access_cpu(gpio_accessen, 1UL);
}
