...
void uart_init(volatile struct LINFlexD_Type* ptr_uart)
{
    ...
    LINFLEXD_SetTxDataFieldLength(ptr_uart, (uint8_t)((uint8_t)uartUserConfig->wordLength >> 1U));
    LINFLEXD_SetRxDataFieldLength(ptr_uart, (uint8_t)((uint8_t)uartUserConfig->wordLength >> 1U));
    ...
    if (uartUserConfig->transferType == LINFLEXD_UART_USING_DMA) {
        LINFLEXD_SetTxMode(ptr_uart, LINFLEXD_UART_RXTX_FIFO_MODE);
        LINFLEXD_SetRxMode(ptr_uart, LINFLEXD_UART_RXTX_FIFO_MODE);
