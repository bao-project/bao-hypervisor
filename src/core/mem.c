...
extern uint8_t _image_start, _image_load_end, _image_end, _vm_image_start, _vm_image_end,
    _data_vma_start;
extern uintptr_t _img_addr, _data_addr;
...
static paddr_t get_root_pool_bitmap_base(void)
{
    ...
    if (!DEFINED(MEM_NON_UNIFIED)) {
        ...
        return (paddr_t)(_img_addr + image_size + vm_image_size + cpu_size);
    } else {
...
static bool pp_reserve_hyp_image_load(struct page_pool* pool)
{
    ...
    struct ppages images_load_ppages = mem_ppages_get(_img_addr, NUM_PAGES(image_load_size));
...
static bool pp_reserve_hyp_image_noload(struct page_pool* pool)
{
    ...
    size_t vm_image_size = (size_t)(&_vm_image_end - &_vm_image_start);
    paddr_t image_noload_addr = _img_addr + image_load_size + vm_image_size;
...
static struct mem_region* mem_find_root_region(void)
{
    ...
    for (size_t i = 0; i < platform.region_num; i++) {
        ...
        bool is_in_rgn;
        if (!DEFINED(MEM_NON_UNIFIED)) {
            is_in_rgn = range_in_range(_img_addr, root_mem_size, region->base, region->size);
        } else {
            is_in_rgn = range_in_range(_data_addr, root_mem_size, region->base, region->size);
        }
...
void mem_init(void)
{
    ...
    if (!all_clrs(config.hyp.colors)) {
        mem_color_hypervisor(_img_addr, root_mem_region);
    }
#endif
