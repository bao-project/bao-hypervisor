 ...
 *
 * @note    The term "Virtual MPU" (vMPU) is a Bao-specific terminology.
 ...
 *
 * Used for ordering MPU entries of the virtual MPU list based on their base addresses.
 *
 * @param _n1 Pointer to first node
 * @param _n2 Pointer to second node
 * @return int 1,-1 or 0, depending on the comparison of nodes' base address.
 ...
 * to the list of MPU entries in the address space configuration.
 *
 * @param as Pointer to the address space.
 * @param mpid MPU entry identifier.
 ...
 *
 * Invalidates the data in a MPU entry, effectively clearing it for reuse.
 *
 ...
 *
 * Clears the entry's contents and marks it as free for reuse.
 * Also removes the entry from the list of MPU entries.
 *
 * @param as Pointer to the address space.
 * @param mpid MPU entry identifier.
 ...
 *
 * @note    If MMIO slave-side protection is supported (MMIO_SLAVE_SIDE_PROT),
 *          the master CPU shall initialize the memory regions in which MMIO devices are mapped.
 *
 ...
 * @see addr_space, as_sec_t, BIT_MASK(), cpumap_t, cpu(), vcpu, vm, SEC_HYP_GLOBAL, SEC_HYP_IMAGE.
 */
 ...
 * @note Optionally, the addition of a new vMPU entry can be broadcasted to all other CPUS.
...
/**
 * @brief Update a vMPU entry in an address space configuration with a different memory region.
...
/**
 * @brief Updates vMPU entry's region to a target region's size.
 *
 * The memory region to update is searched in the vMPU entries and, if found, the entries's
 * region is updated with the updated region's parameters (base address and size).
 *
 * @note Optionally, the update can be propagated via broadcast message to all CPUs.
 *
 ...
 * @see addr_space, mp_region, mpid_t, INVALID_MPID, mpe, list_foreach(),
 *      mem_vmpu_update_region().
 */
...
bool mem_unmap_range(struct addr_space* as, vaddr_t vaddr, size_t size, bool broadcast)
{
    ...
    while (size_left > 0) {
        ...
        if (mpid == INVALID_MPID) {
             ...
             * this to only go ahead with the unmapping if the full range is indeed mapped.
             */
            break;
        }
        struct mpe* mpe = mem_vmpu_get_entry(as, mpid);
...
/**
 * @brief Allocate and map an MPU region in the address space configuration.
 ...
 * @param flags Flag to set for the allocated memory-protected region.
...
/**
 * @brief Allocate and map pages in which a device is memory-mapped.
 * @param as Address space configuration
 ...
 * @param num_pages Number of pages to map.
 * @return vaddr_t Virtual address at which the memory-mapped device is mapped.
